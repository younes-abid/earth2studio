...
"""
Custom Data Loader for Earth2Studio CorrDiff workflows.
Adapted from the main custom_data_loader.py file with ensemble-specific optimizations.
"""
...
class CustomCorrDiffDataSource(DataSource):
    ...
    def __call__(self, time, variable):
        ...
        Returns xarray.DataArray with requested variables and times
        """
    ...
    def _find_time_indices(self, requested_times):
        """Find indices for requested times - properly maps times to data samples"""
        if not hasattr(self, 'time_values') or self.time_values is None:
            print('⚠️  No time coordinate available, using sequential indices')
        ...
        for req_time in requested_times:
            ...
            # Find closest time index in the loaded data
            time_diffs = np.abs(self.time_values - req_time_np)
            closest_idx = np.argmin(time_diffs)
            
            # Validate the match
            min_diff = time_diffs[closest_idx]
            if min_diff > np.timedelta64(1, 'D'):
                print(f'⚠️  Warning: Large time difference for {req_time}')
                print(f'    Requested: {req_time}, Found: {self.time_values[closest_idx]}')
            
            print(f'🎯 Time mapping: {req_time} -> index {closest_idx} ({self.time_values[closest_idx]})')
